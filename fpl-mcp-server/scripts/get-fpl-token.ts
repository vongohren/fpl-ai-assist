 ...
 * FPL Token Fetcher
 *
 * Opens a browser window for you to log in to FPL, then captures the
 * X-Api-Authorization token and saves it to ~/.fpl/secrets.env
 *
 * Usage:
 *   npx tsx scripts/get-fpl-token.ts
 *   # or
 *   npm run get-token
 */
...
async function main() {
  console.log("🔐 FPL Token Fetcher");
  console.log("====================\n");
  console.log("A browser window will open. Please log in to FPL.");
  console.log("The token will be captured automatically after login.\n");
...
function saveToken(token: string) {
  ...
  const content = `# FPL API Token (auto-generated by get-fpl-token)
# Re-run 'npm run get-token' when this expires
export FPL_X_API_AUTH="${token}"
`;

  writeFileSync(FPL_SECRETS_FILE, content, { mode: 0o600 }); // Read/write only for owner
  console.log(`Saved token to ${FPL_SECRETS_FILE}`);
}
